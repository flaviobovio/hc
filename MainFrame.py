...
class MainFrame( gui.MainFrameBase ):
    ...
    def HabilitarControles(self, acti): # True Activar, False Desactivar
        ...
        self.m_staticTextInfo.SetForegroundColour('DARK_GREY')
    ...
    def m_OnButtonClickExportar( self, event ):
        if self.m_Paciente != None:
            acep = wx.MessageBox("Exportar Ficha Paciente ?", "Confirma", wx.YES_NO)
            if (acep == wx.YES):
                archivo = ('fichas/' + self.m_Paciente.nombre + '.doc')

                #os.system("del " + archivo)
                saveout = sys.stdout                  
                fsock = open(archivo, 'w')                             
                sys.stdout = fsock                                       
        
                print "DATOS PACIENTE"
                print
                print "Nombre: " + self.m_Paciente.nombre.encode('utf-8')
                print "Fecha Nacimiento: " + self.m_Paciente.fecha_nacimiento.strftime('%d/%m/%y')
                print "Direccion: " + self.m_Paciente.direccion.encode('utf-8')
                print "Localidad: " + self.m_Paciente.localidad.encode('utf-8') + " - " + self.m_Paciente.provincia.encode('utf-8')
                print "Telefono: " + self.m_Paciente.telefono.encode('utf-8')
                print "Obra Social: " + self.m_Paciente.obra_social.encode('utf-8')
                print "Antecedentes Familiares: " + self.m_Paciente.antecedentes_fam.encode('utf-8')
                print "Antecedentes Personales: " + self.m_Paciente.antecedentes_per.encode('utf-8')
                print
                print 
                print "VISITAS"
                for vis in self.m_Paciente.visitas:
                    print
                    print "Fecha: " + vis.fecha.strftime('%d/%m/%y')
                    print "Motivo: " + vis.motivo.encode('utf-8')
                    print "Detalle: " + vis.detalle.encode('utf-8')
